...
def evaluation_group_probablity(all_output_array, all_test_Y, center_location_list,train_user):
    count, all_recall1, all_recall2, all_recall3, all_recall4, all_recall5, alldistance = 0.,0.,0.,0.,0.,0.,0.
    p=[]
    for j in range(len(all_test_Y)):
        y_test = all_test_Y[j]
        ...
        for i in range(len(y_test)):
            ...
            if flag:
                ...
                if true_pl in infe_pl[1:].argsort()[-15:][::-1]: all_recall4 += 1
                if true_pl in infe_pl[1:].argsort()[-20:][::-1]: all_recall5 += 1
                top20 = infe_pl[1:].argsort()[-20:][::-1]
                if true_pl in top20: p += (21-top20.index(true_pl))/21
                else: p += 1/50
                count += 1
    print(count)
    print([all_recall1,all_recall2,all_recall3, all_recall4, all_recall5])
    print([all_recall1 / count, all_recall2 / count,
           all_recall3 / count, all_recall4 / count, all_recall5 / count, alldistance / count,p])
    # return [all_recall1 / count, all_recall2 / count,
    #         all_recall3 / count, all_recall4 / count, all_recall5 / count, alldistance / count,p]
    return p
...
def load_wordvec(vecpath = WORD_VEC_PATH):
    word_vec = {}
    with io.open(vecpath, encoding='utf-8',mode='r') as f:
        for l in f:
            vec = []
            attrs = l.replace('\n','').split(' ')
            ...
            for i in range(len(features[traj_fea][2])):
                ...
                if not text == 0:
                    ...
                    if dataset=='FS':
                        ...
                        words = text.split('\t')
                    for w in words:
                        if ((w) in text_vec) & (not (w) in useful_vec):
...
def geo_dataset_train_test_text(user_feature_sequence, useful_vec, max_record, place_dim = GRID_COUNT*GRID_COUNT,
    ...
    for u in range(len(user_feature_sequence.keys())):
        user_index[ list(user_feature_sequence.keys())[u]] = u
    user_dim = len(user_feature_sequence.keys())

    word_index = {}
    word_vec = []
    for w in range(len(useful_vec.keys())):
        word_index[ list(useful_vec.keys())[w]] = w
        word_vec.append(useful_vec[ list(useful_vec.keys())[w]])
    word_vec = np.array(word_vec)
    print(word_vec.shape)
    all_train_X_pl, all_train_X_time , all_train_X_user, all_train_X_text,all_train_X_plts , all_train_Y, all_train_evl \
        = [],[],[],[],[],[],[]
    all_test_X_pl, all_test_X_time, all_test_X_user, all_test_X_text,all_test_X_plts, all_test_Y, all_test_evl \
        = [],[],[],[],[],[],[]
    ...
    for user in user_feature_sequence.keys():
        ...
        for sample in range(0,train_size):
            pl_features, time_features, text_features_train ,plts_features\
                = sequ_features[sample][0],sequ_features[sample][1],sequ_features[sample][4],sequ_features[sample][3]
            ...
            plts_train = plts_features[0:len(plts_features)-1]

	    #print("plts shape before")
            #for i in range(len(plts_train)):
	    #	print("%s %s" % (plts_train[i],np.shape(plts_train[i])))
            while len(pl_train) < (max_record-1):
                pl_train.append(0)
                time_train.append(0)
                user_index_train.append(0)
                text_features_train.append(0)
                plts_train.append(np.zeros((GRID_COUNT , GRID_COUNT,1)))

	    #print("plts shape complement")
            #for i in range(len(plts_train)):
	    #	print("%s %s" % (plts_train[i],np.shape(plts_train[i])))
            ...
            all_train_X_plts.append(plts_train)
            #print("plts.shape")
            print(np.shape(all_train_X_plts))
            all_train_Y.append(train_y)
            all_train_evl.append(train_y)

        for sample in range(train_size,len(sequ_features)):
            pl_features, time_features, text_features_test,plts_features_test\
                = sequ_features[sample][0],sequ_features[sample][1],sequ_features[sample][4],sequ_features[sample][3]
            ...
            plts_test = plts_features_test[0:len(plts_features_test) - 1]
            while len(pl_test) < (max_record-1):
                ...
                #plts_test.append(0)
                plts_test.append(np.zeros((GRID_COUNT, GRID_COUNT, 1)))
    ...
    all_train_X_plts = np.array(all_train_X_plts)
    ...
    all_test_X_plts = np.array(all_test_X_plts)

    print ("all_train_X_pl,all_train_X_time,all_train_X_user,all_train_X_text,all_train_X_plts",
           all_train_X_pl.shape,all_train_X_time.shape,all_train_X_user.shape,all_train_X_text.shape,all_train_X_plts.shape)
    return [all_train_X_pl,all_train_X_time,all_train_X_user,all_train_X_text,all_train_X_plts],np.array(all_train_Y), all_train_evl,\
           [all_test_X_pl, all_test_X_time,all_test_X_user,all_test_X_text,all_test_X_plts], np.array(all_test_Y), all_test_evl, \
           user_dim, word_vec, word_index

# create test data for group training
def group_geo_dataset_train_test_text(user_group_weight,model_num,user_feature_sequence, useful_vec, max_record, place_dim = GRID_COUNT*GRID_COUNT,
                              train_test_part=TRAIN_TEST_PART):

    group_weiht = {}
    for u in range(len(user_feature_sequence.keys())):
        group_weiht[ list(user_feature_sequence.keys())[u]] = user_group_weight[u]
    user_dim = len(user_feature_sequence.keys())
    ...
    for user in user_feature_sequence.keys():
        ...
        for sample in range(0,train_size):
            ...
            pl_train = pl_features[0:len(pl_features)-1]
            time_train = time_features[0:len(time_features)-1]
            user_index_train = [(group_weiht[user][model_num] + 1) for item in range(len(pl_features)-1)]
            text_features_train = text_features_train[0:len(text_features_train)-1]
            while len(pl_train) < (max_record-1):
                pl_train.append(0)
                time_train.append(0)
        ...
        for sample in range(train_size,len(sequ_features)):
            ...
            pl_test = pl_features[0:len(pl_features)-1]
            time_test = time_features[0:len(time_features)-1]
            user_index_test = [(group_weiht[user] + 1) for item in range(len(pl_features)-1)]
            text_features_test = text_features_test[0:len(text_features_test) - 1]
            while len(pl_test) < (max_record-1):
                pl_test.append(0)
                time_test.append(0)
    ...
    return [all_train_X_pl,all_train_X_time,all_train_X_user,all_train_X_text],np.array(all_train_Y), all_train_evl,\
           ...
           user_dim, word_vec, word_index,train_user
...
def geo_rnn_train_batch_text(train_X, train_Y, vali_X, vali_Y,vali_evl, model,center_location_list,
    ...
    for i in range(epoch):
        ...
        plts_b = np.array(train_X[4][j:j + BATCH_SIZE])
        print("pl_b,time_b,user_b,text_b,plts_b",
              pl_b.shape, time_b.shape, user_b.shape, text_b.shape,
              plts_b.shape)

        model.fit_generator(batch_generator_text(train_X,train_Y,word_index),steps_per_epoch=int(len(train_X[0])/BATCH_SIZE)+1,
                        epochs=1, max_queue_size=7, validation_data=(vali_X,vali_Y),use_multiprocessing=False,workers=1)
                        #epochs=1, max_queue_size=7, validation_data=(vali_X,vali_Y),use_multiprocessing=False,workers=1)
...
def batch_generator_text(train_X, train_Y,word_index):
    ...
    while 1:
        ...
        while j < train_X[0].shape[0]:
            ...
            print(plts_b.shape)
            #print(plts_b)
            yield ([pl_b, time_b, user_b, text_b,plts_b], np.array(y_b))
	    	...
	    	print("====")
           	print(plts_b.shape)
                ...
                yield ([pl_b, time_b, user_b, text_b, plts_b], np.array(y_b))
                # yield (['pl_input':pl_b, 'time_input':time_b, 'user_input':user_b, 'text_input':text_b,'pltm_input':plts_b], np.array(y_b))
            j = j + BATCH_SIZE
